  ...
  try {
    ...
    if(data && data.data && data.data.klines && data.data.klines.length > 0) {
        ...
        if(Number(passitem[2]) - Number(citem[2]) > 0) {
          log('green', [code,data.data.name, percent+"%",  '3分钟小单流出' , -Number(passitem[2])+Number(citem[2]), '理论值', ltsz * 0.06 / 240 * 3, bk].join(" "));
        }else{
          log('red', [code,data.data.name, percent+"%", '3分钟小单流出' , -Number(passitem[2]) + Number(citem[2]), '理论值', ltsz * 0.06 / 240 * 3, bk].join(" "));
        }
