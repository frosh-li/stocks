...
class Craw {
  constructor() {
    ...
    MongoClient.connect(url, async(err, client) => {
      ...
      this.collection = await this.db.collection("singles", { safe: true });
      //   client.close();
    });
    this.headers = {
      "User-Agent":
  ...
  async startCraw(data) {
    ...
    let url = `https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol=${
      item.symbol
    }&begin=1559923200000&period=day&type=before&count=-1000&indicator=kline`;
    let options = {
      uri: url,
      json: true,
      headers: this.headers,
    ...
    // dates.forEach(async(date, index) => {
        ...
        if (res.length > 0) {
            ...
            if(res2.length > 0) {
                ...
                let nextDate = await collection.find(
                { timestamp: date3, symbol: { $in: symbols },  percent: { $gte: 5, $lte: 10 }},
                );
